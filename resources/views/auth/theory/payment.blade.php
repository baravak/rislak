@section('form')
    @if ($theory->response('status') == 'success')
        <div class="flex flex-col items-center border border-gray-200 shadow-lg rounded-xl px-4 py-6">
            ...
            <a href="{{ Cache::get(request()->route('key'), ['url' => route('dashboard.payments.index')])['url'] }}" class="direct flex items-center text-white text-sm bg-green-500 hover:bg-green-600 transition rounded-full h-8 px-12 mt-8 focus-current ring-green-600 spinner" onclick="_self = this; location.replace = this.href.replace(/^(https|http)/i, '{{ config('app.env') == 'local' ? 'bisloo' : 'risloo' }}'); setTimeout(function(){location.href = _self.href}, 500); return false">@lang('Continue the process')</a>
        </div>
    @else
        <div class="flex flex-col items-center border border-gray-200 shadow-lg rounded-xl px-4 py-6">
            <div class="flex flex-col items-center cursor-default">
            ...
            <a href="{{ route('dashboard.payments.index') }}" class="direct flex items-center text-white text-sm bg-gray-500 hover:bg-gray-600 transition rounded-full h-8 px-12 mt-8 focus-current ring-gray-600 spinner" onclick="_self = this; location.replace = this.href.replace(/^(https|http)/i, '{{ config('app.env') == 'local' ? 'bisloo' : 'risloo' }}'); setTimeout(function(){location.href = _self.href}, 500); return false">@lang('Return to dashboard')</a>
        </div>
    @endif
@endsection
@section('auth-nav')
