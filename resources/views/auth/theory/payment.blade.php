@section('form')
    @if ($theory->response('status') == 'success')
        <div class="flex flex-col items-center border border-gray-200 shadow-lg rounded-xl px-4 py-6">
            ...
            <a href="{{ Cache::get(request()->route('key'), ['url' => route('dashboard.payments.index')])['url'] }}" class="direct flex items-center text-white text-sm bg-green-500 hover:bg-green-600 transition rounded-full h-8 px-12 mt-8 focus-current ring-green-600 spinner" data-mobileLink="{{ config('app.mobileUrl') . 'payment?authorized_key=' . request()->route()->parameter('key') . ($theory->response('billing_id') ? '&billing_id='.$theory->response('billing_id') : '')}}">@lang('Continue the process')</a>
        </div>
    @else
        <div class="flex flex-col items-center border border-gray-200 shadow-lg rounded-xl px-4 py-6">
            <div class="flex flex-col items-center cursor-default">
            ...
            <a href="{{ route('dashboard.payments.index') }}" data-mobileLink="{{ config('app.mobileUrl') . 'payment?authorized_key=' . request()->route()->parameter('key') . ($theory->response('billing_id') ? '&billing_id='.$theory->response('billing_id') : '')}}" class="direct flex items-center text-white text-sm bg-gray-500 hover:bg-gray-600 transition rounded-full h-8 px-12 mt-8 focus-current ring-gray-600 spinner">@lang('Return to dashboard')</a>
        </div>
    @endif
@endsection
@section('auth-nav')
