    ...
    <td class="px-3 py-2 whitespace-nowrap" :class='{"opacity-50" : !enabled}'>
        <div class="flex items-center">
            ...
            <div class="relative mr-1">
                <input type="number" class="text-left ltr w-24 h-7 border border-gray-300 rounded text-xs text-gray-600 focus disabled:bg-gray-100" name="billing[{{ $transaction->id }}]" :value="room_commission" x-model="room_commission" x-effect="room_commission = parseInt(room_commission); room_commission = room_commission = amount > 0 ? room_commission : (room_commission > 0 ? room_commission * -1 : room_commission);room_commission = room_commission = amount > 0 ? Math.round(Math.max(0,Math.min(room_commission, amount))) : Math.round(Math.min(0,Math.max(amount, room_commission)));$el.value = room_commission;commission = Math.round((room_commission * 100) / amount);$dispatch('sheetchange')" x-ref="room_commission" :disabled="!enabled" data-amount="{{ $transaction->amount }}" data-commission="">
