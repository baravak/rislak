<div class="flex flex-col sm:flex-row sm:items-center bg-gray-50 hover:bg-gray-100 transition py-3 pb-4 sm:py-2 p-2 rounded mt-2" x-data="{allowed: {{ $assessment->allowed ? 'true' : 'false' }}, amount:{{ $assessment->amount ?: '0' }}}">
    ...
    {{-- @endif --}}
        <template x-if="allowed">
            <div class="w-20 dir-ltr text-left px-2 hidden sm:flex" x-data="" >
                <button x-lijax:click data-method="DELETE" data-action="{{ route('dashboard.center.assessments.index', [$center->id]) }}/{{ $assessment->assessment->id }}" class="text-gray-600 hover:text-red-600 transition mr-4" title="حذف آزمون" x-on:statio-done="allowed = false; amount = 0">
