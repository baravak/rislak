...
<a href="{{ config('app.server_url')}}/api/command/assessment/fill/{{substr($sample->id, 1)  . '?replace=on' }}" class="inline-flex items-center px-4 h-8 text-xs text-gray-500 hover:text-brand hover:border-brand border border-gray-500 rounded-full transition lijax status-action ml-1 focus">
        {{ __('Fill in') }}
</a>
