<div>
    ...
    <select class="border border-gray-500 h-10 rounded pl-4 pr-8 w-full text-sm focus:border-brand focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-opacity-60" name="field" x-on:change="amount = parseInt($el.querySelector('option[value=\''+$el.value+'\']').getAttribute('data-amount')); if(document.querySelector('#giftCheck')) {document.querySelector('#giftCheck').dispatchEvent(new CustomEvent('link'))}">
