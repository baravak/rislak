<div x-show="amount !== null">
    <div class="mt-4">
        ...
        <div class="flex flex-col xs:flex-row">
            ...
            <button id="giftCheck" data-lijax data-action="/dashboard/giftCheck" x-on:link="$el.setAttribute('data-action', '/dashboard/giftCheck/{{ $center->id }}-'+(gift_code || '').replace(/^{{ $center->id }}\-/, '')+'?region={{ $center->id }}&amount=' + amount); if(gift.title){gift.amount = gift.type == 'percent' ? Math.max(0, (amount * (100 - gift.value))/100) : Math.max(0, amount - gift.value)}" class="mt-2 xs:mt-0 xs:mr-2 flex-shrink-0 text-sm text-white px-4 h-10 rounded bg-green-600 hover:bg-green-700 transition focus-current ring-green-600" data-lijax x-on:statio-done="gift = event.detail; gift_type = event.detail.type;">@lang('اعمال کد تخفیف')</button>
        </div>
    </div>
    <div class="mt-6 pt-6 border-t border-dashed border-gray-300 cursor-default" x-show="gift && gift.title">
        <div class="flex flex-col xs:flex-row xs:items-center justify-between">
            ...
            <span class="text-sm text-gray-500 variable-font-medium mt-0.5 text-left">
