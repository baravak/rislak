<div class="flex items-center justify-between mt-8">
    <div class="relative dropdown">
        <button role="button" class="text-sm text-gray-500 border border-gray-500 rounded-full h-8 w-8 sm:w-auto sm:px-4 hover:bg-gray-100 transition focus-current ring-gray-500 dropdown-toggle" data-toggle="dropdownx" aria-haspopup="true">
    ...
    <div class="flex items-center">
        ...
        <a href="{{ request()->create(isset($room) || $center->type =='personal_clinic' ? route('dashboard.room.schedules.index', ['room' => $room->id]) : route('dashboard.center.schedules.index', ['center' => $center->id]), 'GET', array_merge(request()->all(), ['time'=>$weeks->fri->timestamp + (60*60*30)]))->getUri() }}" role="button" class="flex items-center justify-center flex-shrink-0 text-sm text-brand border border-brand hover:text-white hover:bg-brand rounded-full w-8 sm:w-auto h-8 sm:px-4 transition focus">
